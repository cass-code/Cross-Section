iv_father <- function(data){
  
  source("code/basic_clean.R")
  clean <- basic_clean(data)
  iv1<- clean %>% select(income, age, age2, school, ter, gender, tertiary, race, father_schl, father_terlvl, father_tert) %>% filter(income >0 & age>0 & school>=0 & tertiary>=0 & father_schl >=0 & father_tert >=0 & race<= 4 & race >=1) %>% 
